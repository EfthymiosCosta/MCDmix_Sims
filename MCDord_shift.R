...
for (seed_num in seeds){
  for (eps_val in eps_vec){
    ...
    for (h_val in h_vec){
      ...
      shift_outs_ord[[count]]$h <- h_val
      shift_outs_ord[[count]]$epsilon <- eps_val
      # Calculate proportion of outliers in h-subset
      shift_outs_ord[[count]]$prop_in_h <- length(intersect(MCDmix$Subset, out_indices))/num_outs
      # Calculate proportion of outliers detected by algorithm
      outs_detected <- which(MCDmix$Mahalanobis > crit_val)
      shift_outs_ord[[count]]$prop_true_outs_detected <- length(intersect(outs_detected, out_indices))/num_outs
      # Calculate proportion of regular observations marked as outliers
      shift_outs_ord[[count]]$prop_false_outs_detected <- length(setdiff(outs_detected, out_indices))/length(outs_detected)
      # Number of outliers among the n*(1-epsilon) observations with the lowest distance
      shift_outs_ord[[count]]$num_smallest_dist <- length(intersect(out_indices, order(MCDmix$Mahalanobis)[1:(n-num_outs)]))
      # Compute KL and MSE
      shift_outs_ord[[count]]$MSE <- mse_mat(rand_sigma, MCDmix$Covariance)
      shift_outs_ord[[count]]$KL <- kl_mat(rand_sigma, MCDmix$Covariance)
      
      saveRDS(shift_outs_ord, file = 'shift_outs_ord.RDS')
